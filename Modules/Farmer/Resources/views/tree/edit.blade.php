<x-dashboard.layout>
    <x-panel title="Edit Trees Details">
        <form action="{{route('tree.update', ['tree' => $tree])}}" method="POST">
            @csrf
            @method('PUT')
            @foreach ($columns as $c)
                @if (! isExcludeToForm($c, $model))
                    @if (isSelectField($c, $model))
                        <x-form.select name="{{$c}}" label="{{getFieldLabel($c)}}">
                            @foreach (getFieldsOption($c, $model) as $key=>$value)
                                <option value="{{$key}}" {{$key == $tree[$c] ? 'selected':''}}>{{$value}}</option>
                            @endforeach
                        </x-form.select>
                    ...
                    @else
                        <x-form.input type="{{getFieldType($c, $model)}}" value="{{$tree[$c]}}" name="{{$c}}" label="{{getFieldLabel($c)}}"/>
                    @endif
                @endif
            @endforeach
