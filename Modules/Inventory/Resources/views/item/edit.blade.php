<x-dashboard.layout>
    <x-panel title="Edit Item Details">
        <form action="{{route('item.update', ['item' => $item])}}" method="POST">
            ...
            @foreach ($columns as $c)
                @if (! isExcludeToForm($c, $model))
                    @if (isSelectField($c, $model))
                        <x-form.select name="{{$c}}" label="{{getFieldLabel($c)}}">
                            @foreach (getFieldsOption($c, $model) as $key=>$value)
                                <option value="{{$key}}" {{$key == $item[$c] ? 'selected':''}}>{{$value}}</option>
                            @endforeach
                        </x-form.select>
                    ...
                    @else
                        <x-form.input type="{{getFieldType($c, $model)}}" value="{{$item[$c]}}" name="{{$c}}" label="{{getFieldLabel($c)}}"/>
                    @endif
                @endif
            @endforeach
